...
const DocumentFormatter = (function() {
  ...
  function autoFormatDocument(text) {
    ...
    italicPhrases.forEach(phrase => {
      ...
      formatted = formatted.replace(regex, '$1$2<em>$3</em>');
    });
  ...
  function formatParagraphsHtml(rawText) {
    ...
    return lines.map(line => {
      // Nếu dòng là tiêu đề điều khoản hoặc danh mục số thứ tự: 1. Về mục tiêu...
      let styledLine = line;
      if (/^(\d+\.|\bĐiều\s+\d+\.|\bChương\s+[IVXLCDM]+\.)/i.test(line)) {
